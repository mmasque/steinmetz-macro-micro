function [ tpm, state_counter] = build_tpm_equalStates_randomsampling(fly_data, tau, n_values, n_samples)
...
for state_current = 1 : n_states
    ...
    if length(state_positions) < n_samples
        disp(['number of state occurrences (' num2str(length(state_positions)) ') < n_samples']);
        disp(strcat('For state ', num2str(state_current)));
        continue;
        %error(['number of state occurrences (' num2str(length(state_positions)) ') < n_samples']);
    end
    
    % get a random subset of transitions of length n_samples
    state_positions_random = datasample(state_positions(1:end-1), n_samples, 'Replace', false)';
    % Iterate through occurrences of state and track transitions
    for position = state_positions_random
